...
- **jsPDF darf NICHT als Top-Level-Import stehen** — `import { jsPDF } from "jspdf"` auf Modulebene crasht Hermes beim Tab-Load mit `Unknown encoding: latin1` (Hermes unterstützt nur UTF-8 in TextDecoder). Immer dynamisch importieren: `const { jsPDF } = await import("jspdf")` innerhalb der async Export-Funktion + `patchTextDecoderLatin1()` davor aufrufen. `import type { jsPDF as JsPDFType }` ist für Typ-Annotationen weiterhin OK (kein Runtime-Code).
- **"Fahrterkennung gestoppt"-Banner in tracking.tsx nicht zeigen** — der DRIVE_DETECT_TASK läuft während einer aktiven Fahrt absichtlich nicht (Trip läuft bereits) → Banner zeigt false-positive. Banner komplett entfernt aus tracking.tsx.
- **SaveTripSheet Modal: onRequestClose muss funktionieren** — `onRequestClose={() => {}}` blockiert den Android-Back-Button → App eingefroren. Immer `discardPendingTrip()` mit Alert-Bestätigung einbinden. Außerdem TripRouteMap innerhalb von SaveTripSheet zusätzlich in ErrorBoundary wrappen (TripRouteMap.native.tsx hat intern eine, aber als Double-Safety in SaveTripSheet ebenfalls).
